...
namespace YoutubeExtractor
{
    ...
    public static class DownloadUrlResolver
    {
        ...
        private static string GetPageSource(string videoUrl,System.Net.WebProxy proxy, System.Text.Encoding enc)
        {
#if NETFX_CORE
        ...
        private static IEnumerable<VideoInfo> GetVideoInfos(IEnumerable<Uri> downloadUrls, string videoTitle,string thumnailUrl)
        {
        ...
        private static string GetThumnailUrl(string pageSource)
        {
            string thumnail = null;

            try
            {
                const string thumnailPattern = @"\<meta property=""og:image"" content=""(?<thumnail>.*)""\>";
                var thumnailRegex = new Regex(thumnailPattern, RegexOptions.IgnoreCase);
                Match thumnailMatch = thumnailRegex.Match(pageSource);

                if (thumnailMatch.Success)
                {
                    thumnail = thumnailMatch.Groups["thumnail"].Value;
//#if NETFX_CORE
//                    videoTitle = WebUtility.HtmlDecode(videoTitle);
//#else
//                    thumnail = HttpUtility.HtmlDecode(thumnail);
//#endif

//                    // Remove the invalid characters in file names
//                    // In Windows they are: \ / : * ? " < > |
//                    thumnail = Regex.Replace(thumnail, @"[:\*\?""\<\>\|]", String.Empty);
//                    thumnail = thumnail.Replace("\\", "-").Replace("/", "-").Trim();
                }
            }
